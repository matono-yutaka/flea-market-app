...
<div class="all-contents">
    <div class="left-contents">
        ...
        @foreach($purchases as $sellItem)
            @if ($sellItem->id !== $purchase->id)
                <a class="trading-items" href="{{ url('/chat/' . $sellItem->id) }}">{{$sellItem->item->name}}</a>
            @endif
        @endforeach
    </div>
    <div class="right-contents">
        <div class="upper-contents">
            <div class="image-box">
                <img src="{{ isset($purchase->user->profile) && $purchase->user->profile->image ? asset('storage/images/national_flag/' . $purchase->user->profile->image) : asset('storage/images/20200501_noimage.jpg') }}" alt="" class="profile-image">
            <h1 class="user-name">「{{$purchase->user->name}}」さんとの取引画面</h1>
            </div>
        </div>
...
@if(!is_null($purchase->rating_buyer))
<script>
    location.hash = 'modal1';
</script>
@endif
